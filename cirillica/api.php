...
switch($mode) {
	case "registration": 
	{
		$first_name = guard('first_name', $_GET['first_name'], "regexp", $regexp_name);
		$last_name = guard('last_name', $_GET['last_name'], "regexp", $regexp_name);
		$password = guard('password', $_GET['password'], "regexp", $regexp_password);
	...
	{
		$id = guard('id', $_GET['id'], "regexp", $regexp_id);
		$result = selectDB("SELECT name, photos, description, timing, actions, actors, author, age, intermissions  FROM performance WHERE visible = 1 AND deleted = 0 AND id_performance = $id");
		if($result != 0) {
			$json_success["data"] = $result;
			$price = selectDB("SELECT prices FROM session WHERE id_performance = $id");
			$json_success["data"][0]["price"] = $price[0]["prices"];
			$id = $json_success['data'][0]['actors'];
			$id = explode(",", $id);
			$actors = [];
			for($j = 0; $j < count($id); $j++) {
				array_push($actors, selectDB("SELECT name, photo,(SELECT specialty FROM category WHERE id_category = (SELECT id_category FROM workers WHERE id_category = $id[$j])) as speciality FROM workers WHERE id_worker = $id[$j]")[0]);
			}
			$json_success["data"][0]["actors"] = $actors;
			
			$author = $json_success["data"][0]["author"];
			$id = explode(",", $author);
			$author_db = [];
			for($i = 0; $i < count($author); $i++) {
				array_push($author_db, selectDB("SELECT name, photo FROM workers WHERE id_worker = $author[$i]")[0]);
			}
			$json_success["data"][0]["author"] = $author_db;
			print(json_encode($json_success));
		}
		else print(json_encode($json_error));
		exit(0);
	...
	if($_FILES['picture']['name'][0] != null & isset($_FILES['picture'])) {
		...
		for($i = 0; $i < $total; $i++){
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				...
				//print($_FILES['picture']['tmp_name'][$i])."<br>";
