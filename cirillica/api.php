...
switch($mode) {
	...
	case "repertoire": 
	{
		$result = selectDB("SELECT id_perfomance, name, photos, timing FROM perfomance WHERE visible = 1 AND deleted = 0");
		if($result != 0) {
			$json_success["data"] = $result;
			for($i = 0; $i < count($result); $i++) {
				$json_success["data"][$i]["photos"] = explode(";", $json_success["data"][$i]["photos"])[0];
				$id = $json_success["data"][$i]["id_perfomance"];
				$price = selectDB("SELECT prices FROM session WHERE id_perfomance = $id");
	...
	{
		$id = guard('id', $_POST['id'], "regexp", $regexp_id);
		$result = selectDB("SELECT name, photos, description, timing, actions, actors  FROM perfomance WHERE visible = 1 AND deleted = 0 AND id_perfomance = $id");
		if($result != 0) {
			$json_success["data"] = $result;
			for($i = 0; $i < count($result); $i++) {
				$price = selectDB("SELECT prices FROM session WHERE id_perfomance = $id");
	...
	case "sessions": 
	{
		$result = selectDB("SELECT id_perfomance, id_session, date, time, prices FROM session");
		if($result != 0) {
			for($i = 0; $i < count($result); $i++) {
				$id = $result[$i]["id_perfomance"];
				$data = selectDB("SELECT name, photos, timing FROM perfomance WHERE visible = 1 AND deleted = 0 AND id_perfomance = $id");
				if($data != 0) {
